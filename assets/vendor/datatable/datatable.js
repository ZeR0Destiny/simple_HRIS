    ...
    if (this.headings && this.headings.length) {
      this.headings.forEach((th, i) => {
        ...
        if (
          ...
          th.firstElementChild.classList.contains("dataTable-sorter")
        ) {
    ...
    if (this.options.rowNavigation) {
      ...
      this.body.addEventListener("mousedown", (event) => {
        if (this.table.matches(":focus")) {
          const row = Array.from(this.body.rows).find((row) =>
      ...
      this.pages = rows
        .map((tr, i) =>
          i % this.options.perPage === 0
            ? rows.slice(i, i + this.options.perPage)
            : null
        )
        .filter((page) => page);
    } else {
        ...
        if (rows.length) {
          if (options.headings) {
            ...
            if (options.removeDoubleQuotes) {
              obj.headings = obj.headings.map((e) =>
                e.trim().replace(/(^"|"$)/g, "")
              );
            }
            rows.shift();
